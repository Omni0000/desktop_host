...
fixtures! {
	...
	plugins  = { consumer: "consumer", counter: "counter", counter_sync: "counter-sync" };
}

#[test]
fn native_async_resource_imports_accept_a_sync_plugin() -> Result<(), Box<dyn std::error::Error>> {
	futures::executor::block_on( async {
		let engine = Engine::default();
		let linker = Linker::new( &engine );
		let plugins = fixtures::plugins( &engine );
		let bindings = fixtures::bindings();
		let counter = plugins.counter_sync.plugin.instantiate( &engine, &linker )?;
